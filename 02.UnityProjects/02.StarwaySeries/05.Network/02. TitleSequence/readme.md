...
11. [반복되는 설계 패턴](#11-반복되는-설계-패턴)
12. [코드에서 발견한 주의점](#12-코드에서-발견한-주의점)
13. [근거 / 출처](#13-근거--출처)
...
이 파일 전체(6개 게이트 중 5개)에서 동일한 두 가지 패턴이 반복됩니다. 한 번만 이해하면 나머지 게이트를
읽는 속도가 크게 빨라집니다.

### 패턴 A. `isFinished` + `WaitUntil` 콜백 동기화

```csharp
bool isFinished = false;
// ... 비동기 콜백 어딘가에서 isFinished = true; 로 세팅 ...
yield return new WaitUntil(() => isFinished);
```

Unity 코루틴은 `yield return 콜백기반API`를 직접 지원하지 않습니다. 이 파일은 지역 변수
`isFinished`를 클로저로 캡처해 콜백 안에서 세팅하고, 코루틴 쪽은 `WaitUntil`로 매 프레임 그 값을
폴링하는 방식으로 "팝업 확인을 누를 때까지 대기", "웹뷰가 닫힐 때까지 대기" 같은 흐름을
`IEnumerator` 체인 안에 자연스럽게 녹여냈습니다. 모든 실행 경로(성공/실패/스킵)가 반드시
`isFinished = true`로 수렴하도록 짜여 있어야 다음 게이트로 넘어간다는 점이, 이 패턴을 읽을 때
가장 먼저 확인해야 할 불변조건입니다(5번 게이트인 `CheckAppUpdate`의 강제 업데이트 분기가 유일한
예외입니다 — 8절 참고).

### 패턴 B. `#if UNITY_EDITOR` 개발 편의 분기

`CheckAppUpdate`, `CheckPopupUrl`, 약관 동의 로직 세 곳 모두 에디터에서는 실기기 전용 동작(스토어
이동, 네이티브 웹뷰, 앱 종료)을 팝업이나 즉시 콜백으로 대체합니다. 분기 조건의 나머지 절반(allow /
isSkipAllow 판단)은 에디터·실기기가 완전히 동일한 로직을 타므로, **에디터에서 검증한 분기 결과를
그대로 신뢰할 수 있는 구조**입니다.
