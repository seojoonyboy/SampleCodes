...
> 파이프라인을 코드 레벨로 분석한 문서이다. 대상 파일은 `Assets/Scripts/Server/Network/NetworkManager.cs`
...
> CheckResourceServerUrl) → 약관 동의` 체인만 발췌했다.

**대상 파일**: `Assets/Scripts/Server/Network/NetworkManager.cs` (이 폴더의 [`NetworkManager.cs`](./NetworkManager.cs))
...
필요한지", "지금 공지할 내용이 있는지", "약관에 다시 동의를 받아야 하는지"를 차례로 판단해야 한다.
이 판단들을 클라이언트가 한 번에 병렬로 물어보면, 예를 들어 강제 업데이트 대상인 유저에게 공지 팝업이나
약관 동의 화면이 먼저 떠버리는 순서 역전이 발생할 수 있다. `CheckVersionData()`가 6개 게이트를
`IEnumerator`로 하나씩 직렬 연결해 둔 것은 이 순서 보장이 핵심 목적이다. 배치는 IP 조회 → 강제 업데이트 → 공지 → 웹뷰 공지 → 최종 허용 → 리소스 서버 순이며,
앞 단계에서 걸러질 유저(예: 강제 업데이트 대상)가 뒤 단계의 공지 팝업이나 약관 화면을 먼저 보는 순서 역전을 막는다. 이 체인을 모두
통과한 뒤 `callback`이 호출되면 로그인 화면에서 SSO 웹뷰 로그인(`ShowLoginWebView` / `SBHttp.RequestSSO`)
흐름이 이어지는데, 이 문서는 그 앞단인 게이트 체인 자체에 집중한다.
...
`isSkipAllow`는 **Gate 1**에서 한 번 계산되어 **Gate 3, 4, 5** 세 곳에서 재사용되는 공유 상태이다. 이 문서
전체에서 반복해서 등장하니 먼저 짚고 넘어간다.
...
모두 `access`/`version` 하나로부터 파생된 값이라 이론적으로는 동시에 판단할 수도 있다. 그럼에도
`yield return`으로 하나씩 순서를 강제한 이유는 두 가지이다. 첫째, 강제 업데이트 대상 유저가 공지
팝업이나 약관 동의 화면을 먼저 마주치는 순서 역전을 막기 위함이고, 둘째, `isSkipAllow`처럼 뒤 게이트가
앞 게이트의 계산 결과를 그대로 참조하는 의존 관계가 있기 때문이다. 병렬로 실행했다면 이 의존 관계를
게이트마다 다시 동기화해야 했을 것을, 직렬 코루틴 체인이라는 가장 단순한 도구로 해결한 것이다.
...
- `Open()`은 `NetworkManager`의 단일 진입점이다. `isStarted` 플래그를 세우고, 자동 로그인용
  `AccessInfoURL`을 `SBConfigs`에 주입한 뒤, **토큰 자동 갱신 코루틴**(`StartRefreshTokenCoroutine`, 60초
  주기)을 먼저 띄우고 나서야 `InitProcess`로 넘어간다. 즉 부팅 게이트 체인이 아직 끝나지 않았어도 토큰
  갱신 루프는 이미 백그라운드에서 돌기 시작한다. 이 순서를 뒤집어 게이트 체인이 끝난 뒤에 갱신
  코루틴을 띄웠다면, 게이트 통과에 오래 걸리거나 유저가 공지 팝업을 붙들고 있는 동안 로그인 이후 발급된
  토큰의 첫 갱신 시점이 그만큼 늦어질 수 있다 — 부팅 절차와 무관하게 최대한 일찍 갱신 루프를 확보해
  두는 편이 안전하다는 판단으로 읽힌다.
- `InitProcess`는 `SBHttp.RequestAccess`로 서버에서 **접속 정보 전체**(`AccessDto`, 여러 버전대의 정책이
  배열로 들어있음)를 한 번에 받아온다. 통신 자체가 실패하면(`access == null`) 이후 게이트를 아예 태우지
  않고 `OpenNetworkUnAvailablePopup()`으로 조기 종료한다.
- 흥미로운 부분은 **버전 범위 매칭 루프**이다. `access.versions` 배열의 각 항목은 `min`/`max` 버전
  문자열을 가지고 있고, `SBString.versionDiff(a, b)`로 두 버전 문자열의 대소를 비교한다.
  `minDiff + maxDiff > 0`이라는 조건은 "현재 버전이 `min` 이상이면서 `max` 이하"인 구간을 찾는
  트릭이다 — `min`이 없으면 무조건 통과(`1`), `max`가 없으면 무조건 통과(`1`)로 처리해 하한/상한이
  없는 버전 정책도 표현할 수 있게 했다. 여러 버전대(예: 1.0.x, 1.1.x, 1.2.x)마다 **강제 업데이트
  여부, 접속 서버, 공지 문구를 서버 배열 하나로 동시에 관리**할 수 있는 구조이다.
- 매칭된 `version`이 `null`일 수도 있는데(현재 버전이 어떤 구간에도 속하지 않는 경우), 이 경우의 처리는
  다음 단계인 `CheckVersionData`의 첫 줄에서 이루어진다.
...
- 이 함수는 실제 판단 로직을 전혀 갖고 있지 않은 **오케스트레이터**이다. 6개의 하위 코루틴을
  `yield return`으로 순차 연결만 한다. Unity 코루틴에서 `yield return 다른IEnumerator`는 "그
  코루틴이 완전히 끝날 때까지 여기서 대기"를 의미하므로, 이 6줄만으로 **엄격한 순서 보장**이 이루어진다
  — IP 체크가 끝나기 전에는 강제 업데이트 팝업이 뜨지 않고, 강제 업데이트를 통과하기 전에는 공지 팝업이
  뜨지 않는다.
- `version == null`이면 `yield break`로 **코루틴 자체를 즉시 종료**한다. 이 경우 `callback`이 영원히
  호출되지 않는다는 점에 주의해야 한다 — 로그로만 남기고 사실상 게임이 로그인 화면으로 진행하지 못하고
  멈춘다(서버 접속 정보 배열에 현재 버전을 커버하는 구간이 없는, 설정 실수 상황에 대한 방어).
- 각 하위 게이트 앞에 붙은 한국어 주석(`// 아이피 체크.`, `// 앱 강제업데이트 체크.` 등)이 곧 이
  함수의 "목차" 역할을 한다 — 코드 자체가 자기 문서화된 형태이다.
...
한다. 이 우회 판단은 **외부 IP 조회 결과**로 이루어지는데, `access.ipCheckUrl`을 호출하는 이 조회
자체가 네트워크 장애나 응답 지연으로 실패할 가능성이 있고, 그렇다고 이 실패가 전체 부팅 시퀀스를 멈춰서는
안 된다.

**해결 과정**

- `WebClient().DownloadString(access.ipCheckUrl)`로 외부 IP 조회 API를 **동기 호출**한다(코루틴
  안이지만 이 한 줄 자체는 블로킹 호출이다 — 응답이 올 때까지 해당 프레임 실행이 멈춘다).
- 응답으로 받은 IP가 `access.allowIPs`(서버가 내려준 화이트리스트 배열) 안에 있는지
  `Array.Exists`로 확인해 `isSkipAllow`에 대입한다.
- 이 호출 전체를 `try/catch`로 감싸, 예외가 나도 `isSkipAllow = false`(우회 아님, 즉 "안전하게 일반
  유저처럼 취급")로 폴백하고 `isFinished = true`로 코루틴을 마무리시킨다. **성공 경로와 실패 경로
  모두 반드시 `isFinished = true`로 귀결**되도록 짜여 있어, 이 함수가 무한정 대기하는 경우가 없다.
...
- `yield return new WaitUntil(() => isFinished)`는 이 파일 전체에서 반복되는 **코루틴 ↔ 콜백 동기화
  패턴**이다(이후 각 게이트에서도 같은 형태가 반복된다). 비동기 콜백이 `isFinished`를 세팅할 때까지 매 프레임 조건을 검사하며 대기한다.

**결과**

이후 3~5번 게이트(공지 팝업, 웹뷰 공지, 최종 허용 판단)에서 공통으로 참조하는 `isSkipAllow` 플래그를
한 곳에서 확보했고, 조회 API 장애가 전체 부팅을 막는 단일 장애점(SPOF)이 되지 않도록 격리했다.
...
치명적 버그나 정책 변경 시 서버 값 하나로 구버전 클라이언트의 실행 자체를 막아야 한다. 그런데
에디터에서 개발/QA할 때마다 스토어로 튕기거나 플레이가 강제 종료되면 개발 루프가 크게 방해받는다.

**해결 과정**

- `version.needToBeUpdated`가 `true`인 경우에만 분기한다. 팝업 확인 콜백 안에서
  ...
  실기기 빌드에서는 `Application.Quit()`으로 갈라 처리한다.
- **눈여겨볼 지점**: `needToBeUpdated` 분기에서는 `isFinished`를 **어디에서도 `true`로 세팅하지
  않는다.** `yield return null`로 한 프레임만 양보하고 그 다음 줄인
  `yield return new WaitUntil(() => isFinished)`로 넘어가는데, 이 조건은 영원히 거짓이다. 즉 이
  분기를 타면 **코루틴은 사실상 다시는 재개되지 않는다.** 이건 버그가 아니라, "팝업 확인과 동시에
  앱이 종료되거나 플레이가 멈추므로 그 뒤 코드가 실행될 일이 없다"는 전제를 깔고 짠 의도적인 설계로
  읽힌다 — 다만 `Application.Quit()`이 즉시 프로세스를 죽이지 않는 일부 플랫폼(예: 특정 콘솔/웹
  환경)에서는 이 가정이 깨질 수 있어, 이식성 관점에서는 잠재적 위험 지점이기도 한다.
- `needToBeUpdated`가 `false`면 `isFinished = true`로 즉시 통과.

**결과**

배포 파이프라인을 거치지 않고 서버 값 변경만으로 특정 버전대의 실행을 즉시 차단할 수 있고, 로컬 개발
워크플로우는 이 게이트에 전혀 영향받지 않는다.
...
가져야 한다 — 그냥 안내성 공지라면 확인 즉시 진행, 하지만 `version.allow`가 꺼져 있는 상황(서비스
점검/차단)이라면 같은 확인 버튼이 사실상 "차단 안내를 읽었으니 재시작"의 트리거가 되어야 한다.

**해결 과정**

- 로케일 매칭: `version.popupMessage`는 `{region, text}` 배열이다. 루프를 돌면서 **①`region == "en"`인
  항목을 먼저 기본값으로 채우고, ②사용자 로케일(`GameStorage.UserAccountLanguage`)과 일치하는 항목을
  만나면 덮어쓰는** 2단계 방식으로 "해당 언어 없으면 영문 폴백" 동작을 배열 순회 한 번으로 구현했다.
- 팝업 확인 콜백에서의 3중 분기가 핵심이다:
...
경우가 있다. 그런데 에디터에는 실제 인앱 웹뷰(`UniWebView` 등 네이티브 플러그인)를 띄울 수 없다는
플랫폼 제약이 있다.
...
- 3번 게이트와 **완전히 동일한 3중 allow/isSkipAllow 분기**를 재사용하되, 차단 시 최종 액션만
  ...
  또는 `Application.Quit()`(완전 종료)으로 더 강하게 처리한다 — 3번은 "다시 시작", 4번은 "종료"로
  차단 강도가 다르게 설계되어 있다.
- `#if UNITY_EDITOR ... #else ... #endif`로 에디터/실기기를 완전히 이분화했다. 에디터 분기는 실제
  웹뷰 대신 `ViewController.OpenConfirmPopup`을 재사용하면서, 팝업 메시지 자체를
  `"에디터라서 팝업으로 대체"`라는 개발자를 위한 문구로 남겨 두었다 — 실기기와 100% 동일한 코드
  경로(같은 allow/isSkipAllow 분기)를 타면서도 웹뷰 플러그인 의존성만 우회한 것이다.
- 실기기 분기는 `version.popupUrl`에 언어 쿼리스트링(`?lang=ko`/`?lang=en`)을 붙여
  `ViewController.OpenWebView`를 호출한다.

**결과**

"공지 노출"과 "서비스 차단"을 웹뷰 기반으로도 동일하게 지원하면서, 에디터에서는 네이티브 웹뷰 플러그인
없이도 **같은 분기 로직을 즉시 검증**할 수 있다. 이는 QA/개발 반복 속도에 직접적으로 기여하는
부분이다.
...
3, 4번 게이트는 `popupMessage`/`popupUrl`이 **있을 때만** allow를 검사한다. 즉 서버가 공지 문구나
웹뷰 URL을 아예 내려주지 않으면 그 사이에서는 `allow=false`가 걸러지지 않고 통과해 버린다. 공지 없이
접속만 차단하고 싶은 경우를 위한 **최종 안전장치**가 필요하다.

**해결 과정**

- `!version.allow`(차단 상태)일 때만 동작한다. `isSkipAllow`면 조용히 통과시키고, 아니면
  `LocaleController.GetBuiltInLocale(11)` — 코드에 내장된 로케일 문자열 11번(차단 안내 문구로 추정) —
  을 담은 확인 팝업을 띄운 뒤 확인 즉시 `GameScene.Instance.OnRestart()`로 재시작시킨다.
...
확보했다. 3~5번 게이트가 사실상 "차단 판단을 내리는 3중 체크포인트"를 형성한다.
...
앞선 5개 게이트와 달리 판단 로직이 없는 단순 대입이다. 서버가 내려준 리소스(에셋) 서버 이름을
`AssetServerName` 프로퍼티로 캐싱해, 이후 `TitleScene` 의 다운로드 단계(에셋 목록 요청 이후 `TestAssetLoader()` 등,
[AdditionalResourceDownload.md](../../AdditionalResourceDownload.md) 참고)가 어떤 서버에서 에셋을 받아올지 판단할 수 있게 한다. `yield return null`로 한 프레임만 양보하고 바로
종료되므로 이 게이트는 사실상 대기 시간이 없다.

---

## 10. 마무리 — 버전정보 반영 & 약관 재동의

`CheckVersionData()`의 6개 게이트를 모두 통과한 뒤 실행되는 코드이다.
...
하고, 이 판단을 매번 서버에 묻지 않고 로컬 상태만으로 내려야 한다.

**해결 과정**

- 게이트 통과 이후에는 `null` 체크가 된 필드만 골라서 `SBConfigs`에 반영한다(`loginKey`,
  ...
  방식이다.
- 약관 동의는 `SBConfigs.GetAuthDtoByPlayerPrefs()`로 저장된 인증 여부를 먼저 가른다.
  - 인증이 없으면(신규/게스트) 무조건 약관 웹뷰를 보여준 뒤 `TermsVersion`/`TermsAgreeDate`를
    서버 시간(`SBTime.Instance.ServerTime`) 기준으로 갱신한다.
  - 인증이 있으면 `GameStorage.TermsVersion < access.termsVersion`(약관이 그 사이 갱신됨) **또는**
    ...
    재동의 웹뷰를, 아니면 바로 `callback`을 호출한다.
- 두 분기 모두 `#if UNITY_EDITOR`에서는 웹뷰 없이 `callback?.Invoke()`로 즉시 통과시켜, 에디터
  개발 중에는 약관 동의 UX를 매번 거치지 않도록 했다.

**결과**

약관 재동의 주기라는 정책·법적 요구사항을, 별도 서버 폴링 없이 로컬 상태(`TermsVersion`,
`TermsAgreeDate`) 비교만으로 판단하는 로직으로 내재화했다. 네 가지 유저 케이스가 조건식 두 줄로
정리된다.
...
한계와 개선 방향
------------------
> * **IP 조회가 메인 스레드에서 동기 호출된다.** `CheckGetIP` 는 `new WebClient().DownloadString(...)` 를 코루틴 안에서 그대로 호출하고, 타임아웃을 지정하지 않으며 인스턴스도 해제하지 않는다. 네트워크가 불안정한 콜드 스타트 구간에서 응답이 늦으면 그동안 프레임이 멈출 수 있다. `catch` 도 예외를 기록 없이 삼킨다.
>   기존 `SBHttp`(BestHTTP)로 짧은 타임아웃을 두고 비동기 조회하고, 실패 원인을 로그로 남기는 것이 좋다.
> * **체인의 종료 경로가 명시적이지 않다.** `version == null` 이면 `yield break` 만 하고 `callback` 도 팝업도 없어 타이틀이 로딩 문구에서 멈춘다. `ShowTermsWebView` 의 `cb` 는 동의 성공(`cb(true)`)에서만 호출되므로 유저가 동의 없이 웹뷰를 닫으면 진행되지 않고, `isSucess == false` 분기는 도달할 수 없다. 약관 웹뷰 오류 시에는 횟수 제한 없이 자기 자신을 다시 호출한다.
>   모든 경로가 "callback 호출" 또는 "재시작/종료를 안내하는 명시적 UI" 중 하나로 끝나도록 정리하고, 체인 전체에 상한 시간을 두는 것이 안전하다.
> * **`allow`/`isSkipAllow` 판정이 게이트마다 복사되어 있고 공유 필드에 의존한다.** 같은 3중 분기가 `CheckPopupMessage`, `CheckPopupUrl`(에디터/실기기 각각), `CheckAllow` 에 반복되며 차단 시 동작(재시작/종료)만 다르다. `isSkipAllow` 는 `CheckGetIP` 에서 `ipCheckUrl` 이 없으면 대입되지 않아, `Open()` 이 다시 실행되면 이전 값이 남는다.
>   `(allow, isWhitelisted, needsNotice)` 를 받아 `Pass/Restart/Quit` 를 돌려주는 순수 함수로 분리하면 중복이 사라지고 EditMode 테스트로 검증할 수 있다.
> * **토큰 갱신 코루틴 관리가 멱등적이지 않다.** `Open()` 은 호출될 때마다 `StartRefreshTokenCoroutine()` 을 실행하는데, 리소스 목록 실패 시 타이틀이 `Open()` 을 다시 태우면 이전 코루틴을 멈추지 않은 채 참조만 덮어쓴다. `OnApplicationPause` 는 일시정지 때 시작하고 복귀 때 멈추는 것으로 보여 의도와 반대일 가능성이 있으며, 주석은 "5초에 한 번"이지만 실제 대기는 `60f` 이다.
>   시작 전에 기존 코루틴을 정리하는 가드를 두고, 일시정지/복귀 처리와 주석을 실제 의도에 맞춰 바로잡아야 한다.
> * **매직 값과 중복 로직이 많아 테스트하기 어렵다.** `GetBuiltInLocale(6/7/11)` 인덱스, `"에디터라서 팝업으로 대체"` 문자열, `AddYears(2)`, `"?lang="` 이어붙이기(URL 에 이미 쿼리가 있는 경우를 가정하지 않음)가 코드에 직접 들어 있고, `GameStorage.UserAccountLanguage == ...ko ? "ko" : "en"` 삼항식이 이 클래스에서만 7회 나온다. 약관 동의 블록도 신규/기존 유저 분기에 거의 같은 코드가 두 번 있다. 버전 구간 매칭(`minDiff + maxDiff > 0`)은 `SBString.versionDiff` 의 반환 규약에 의존하는데 이 저장소에는 정의가 없어 경계(`min == max == 현재`) 동작을 확인할 수 없다.
>   로케일 키를 상수/열거형으로, 언어 코드를 헬퍼로 모으고, 버전 범위와 약관 재동의 조건을 별도 함수로 뽑아 경계값 단위 테스트를 붙이는 것이 좋다.
> * **한 클래스에 책임이 과도하게 모여 있다.** `NetworkManager` 는 1,822줄로 부팅 게이트, 에셋 다운로드(`TestAssetLoader`), 로그인, 토큰 갱신, 웹뷰 6종(약관/로그인/연동/내정보/탈퇴/쿠폰), 가챠·랭킹·미션·리듬 등 게임 API 래퍼를 함께 가진다. 게이트 체인이 `ViewController`, `GameScene.Instance`, `Application` 같은 전역에 직접 의존하므로 게이트 로직만 떼어 테스트하기 어렵다.
>   `BootGateSequence` 같은 순수 C# 클래스로 분리하고 HTTP/UI 를 인터페이스로 주입하면 각 게이트를 독립적으로 검증할 수 있고, 다운로더와 API 클라이언트도 별도 파일로 나눌 수 있다.
