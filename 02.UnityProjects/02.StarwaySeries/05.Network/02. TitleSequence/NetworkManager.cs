...
namespace Snowballs.Network
{
	public class NetworkManager : MonoBehaviour
	{
		...
		public Dictionary<FileDto, bool> assetFiles = new Dictionary<FileDto, bool>();
		...
		public IEnumerator CheckVersionData(AccessDto access, VersionDto version, Action callback)
		{
			...
			if (auth == null)
			{
				...
				callback?.Invoke();
					...
					string url = access.terms + "?lang=" + ((GameStorage.UserAccountLanguage == Snowballs.Client.Type.Language.ko) ? "ko" : "en");
					Debug.Log("url = " + url);
		...
		public async void LoginProcess(Action<ResponseDto<SonPlayerDto>, ResponseDto<SonPlayerSubDto>> cb, Action hideCallback)
		{
			...
			device.locale = (Application.systemLanguage == SystemLanguage.Korean) ? "ko" : (Application.systemLanguage == SystemLanguage.Japanese) ? "jp" : "en"; // 디바이스 언어 값
			...
			//	GameStorage.UserAccountLanguage = (Application.systemLanguage == SystemLanguage.Korean) ? Snowballs.Client.Type.Language.ko : Snowballs.Client.Type.Language.en;
			...
			void OpenNicknamePopup()
			{
				...
				Popup.Load("NickName/TextInputPopup", p, (popup, result) =>
				{
					...
					GamePlayer.Init(initReq, (response) =>
					{
						...
						{
							LoadingIndicator.Hide();
							CommonProcessController.OpenTutorialVoiceSelectPopup((isSuccess) =>
							{
								LoadingIndicator.Show();
								GetPlayerInfo((_response) => {
									GetPlayerSubInfo((response2) =>
									{
										LoadingIndicator.Hide();
										cb(_response, response2);
									});
								});
							});
						}
		...
		public void SetPart(RequestDto<CardPartDto> requestDto, Action<ResponseDto<SonCardResultDto>> callback)
		{
			GameCard.SetPart(requestDto, (response) =>
			{
				callback(response);
			});
		}
		...
		public void InAppDiamondCheck(RequestDto<StoreBuyDto> requestDto, Action<ResponseDto<CodeDto>> callback)
		{
			GameStore.InAppDiamondCheck(requestDto, (response) =>
			{
				callback(response);
			});
		}

		public void InAppPopupStoreCheck(
			RequestDto<StoreBuyDto> requestDto,
			Action<ResponseDto<CodeDto>> callback)
		{
			GameStore.InAppPopUpCheck(requestDto, (response) =>
			{
				callback(response);
			});
		}
		...
		public void BuyFromPopUp(RequestDto<StoreBuyDto> requestDto, Action<ResponseDto<AcquiredItemDto>> callback)
		{
			GameStore.BuyFromPopUp(requestDto, (response) =>
			{
				callback(response);
			});
		}
		...
		public void GetRhythmRank(RequestDto<CodeDto> requestDto, Action<ResponseDto<RhythmRankDto>> callback)
        {
			GameRank.GetRhythm(requestDto, (response) =>
			{
				callback(response);
			});
		...
		public void GetWeekdailyMission(Action<ResponseDto<MissionListDto>> callback)
		{
			GameMission.GetWeekdaily((response) =>
			{
				callback(response);
			});
		}

		public void GetEventMission(RequestDto<CodeDto> requestDto, Action<ResponseDto<MissionDto>> callback)
		{
			GameMission.GetEvent(requestDto, (response) =>
			{
				callback(response);
			});
		}

		public void ReceiveMission(RequestDto<CodeValueDto> requestDto, Action<ResponseDto<AcquiredItemDto>> callback)
		{
			GameMission.Receive(requestDto, (response) =>
			{
				callback(response);
			});
		}
		...
		//리듬 즐겨찾기
		public void SetRhythmFavorite(RequestDto<CodeValueDto> requestDto, Action<ResponseDto<String>> callback)
        {
			GameRhythm.SetFavorite(requestDto, (response) =>
			{
				callback(response);
			});
		}
		...
		public void RhythmStart(RequestDto<RhythmStartDto> requestDto, Action<ResponseDto<RhythmResponseDto>> callback)
		{
			GameRhythm.RhythmStart(requestDto, (response) =>
			{
				callback(response);
			});
		}

		public void RhythmContinue(RequestDto<RhythmContinueDto> requestDto, Action<ResponseDto<RhythmResponseDto>> callback)
		{
			GameRhythm.RhythmContinue(requestDto, (response) =>
			{
				callback(response);
			});
		}
		...
		public void RhythmSubmit(RequestDto<RhythmResultDto> requestDto, Action<ResponseDto<RhythmResponseDto>> callback)
		{
			GameRhythm.RhythmSubmit(requestDto, (response) =>
			{
				callback(response);
			});
		}

		/*public void ExchangeReward(RequestDto<PopUpStoreBuyDto> requestDto, Action<ResponseDto<AcquiredItemDto>> callback)
		{
			GameStage.ExchangeReward(requestDto, (response) =>
			{
				callback(response);
			});
		}*/
		...
		public void ShowLoginWebView(Action<ResponseCode, SSOResultResponse> cb)
		{
			SSOAccessRequest accessReq = new SSOAccessRequest()
			{
				...
				type = 0,
				lang = ((GameStorage.UserAccountLanguage == Snowballs.Client.Type.Language.ko)
						? "ko" : "en"),
			};
		...
		public void ShowCouponWebView(string url, Action<bool> cb)
		{
			var webViewGameObject = new GameObject("UniWebView");
			var webView = webViewGameObject.AddComponent<UniWebView>();
			webView.SetShowSpinnerWhileLoading(true);
			
			webView.AddUrlScheme("sbcoupon");
			...
			webView.SetShowToolbar(true, false, true, true);
			webView.SetShowToolbarNavigationButtons(true);
			GameScene.Instance.AddWebViewCloseListener(OnClickClose);
			
#if UNITY_ANDROID
			//Note. 화면의 96% [높이 기준] WebView Frame 설정, 상단 영역은 화면의 4% 차지
			webView.Frame = new Rect(0, Screen.height * 0.04f, Screen.width, Screen.height * 0.96f);
			GameScene.Instance.WebViewTop.gameObject.SetActive(true);
			
			webView.Load(url);
#elif UNITY_IOS
			var userAgent = System.Text.RegularExpressions.Regex.Replace(webView.GetUserAgent(), "(Mobile\\/[A-Za-z0-9]+)$", "Version/14.1.2 $1 Safari/604.1");
			...
			webView.OnPageErrorReceived += (uniWebView, errorCode, errorMessage) =>
			{
				SBDebug.Log("OnPageErrorReceived : " + errorMessage);
			};

			webView.OnPageFinished += (uniWebView, statusCode, url) =>
			{
				if ((statusCode == 200) || (statusCode == 201))
				{
					SBDebug.Log("coupon on pageFinished response code : " + statusCode);
				}
			};

			webView.OnMessageReceived += (view, message) =>
			{
				SBDebug.Log("coupon webview message received : " + message.ToString());
				
				if (message.Scheme == "sbcoupon")
				{
					SBDebug.Log("webView OnMessageReceived Path : " + message.Path);
					...
					{
						string code = message.Args["code"];
						SBDebug.Log("coupon error code = " + code);
			...
			webView.OnShouldClose += (view) =>
			{
				SBDebug.Log("webView OnShouldClose");
			...
			void OnClickClose()
			{
				...
				{
#if UNITY_ANDROID
					GameScene.Instance.WebViewTop.gameObject.SetActive(false);
#endif
					Destroy(webView);
					webView = null;
				}
