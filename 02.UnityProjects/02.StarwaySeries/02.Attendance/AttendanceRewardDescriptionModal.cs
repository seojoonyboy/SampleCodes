...
public class AttendanceRewardDescriptionModal : MonoBehaviour
{
    ...
    public void OnOpen(Params p)
    {
        ...
        this.innerBgRect.sizeDelta = new Vector2(1000, 200 * totalRawNum + 200f);
        ...
        for (int i = 1; i <= totalRawNum; i++)
        {
            ...
            for (int j = 0; j < slotNumPerRaw; j++)
            {
                ...
                if (itemIndex < p.rewardItems.Count)
                {
                    ...
                    numberText.text = "x" + p.rewardItems[itemIndex].amount;
